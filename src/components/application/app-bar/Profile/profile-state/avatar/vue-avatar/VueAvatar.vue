...
export default {
	...
	mounted () {
		...
		if (!this.image) {
			var placeHolder = this.svgToImage(
				`<svg width="150" height="150" viewBox="0 0 1792 1792" xmlns="http://www.w3.org/2000/svg">
				<path style="fill:rgb(131, 145, 135)"
				d="M1596 380q28 28 48 76t20 88v1152q0 40-28 68t-68 28h-1344q-40 0-68-28t-28-68v-1600q0-40 28-68t68-28h896q40
				0 88 20t76 48zm-444-244v376h376q-10-29-22-41l-313-313q-12-12-41-22zm384 
				1528v-1024h-416q-40 0-68-28t-28-68v-416h-768v1536h1280zm-128-448v320h-1024v-192l192-192 128 128 
				384-384zm-832-192q-80 0-136-56t-56-136 56-136 136-56 136 56 56 136-56 136-136 56z"/></svg>`
			)
	...
	methods: {
		...
		svgToImage (rawSVG) {
			...
			// this needed for Create object URL in defferent browser
		...
		setState (state1) {
			// Math.ceil() function always rounds a number up to the next largest
			var min = Math.ceil(1)
		...
		onDrop (e) {
			...
			if (e.dataTransfer && e.dataTransfer.files.length) {
				...
				// document.querySelector('.cursorPointer').style="cursor: all-scroll"
		...
		clicked (e) {
			if (this.dragged === true) {
				this.state.drag = false // my commit
				this.dragged = false
			} else {
		...
		loadImage (imageURL) {
			...
			imageObj.onload = () => {
				let cursor = document.querySelector('.cursorPointer')
				cursor.style="cursor: all-scroll"
				
				let imageState = this.getInitialSize(imageObj.width, imageObj.height)
		...
		getCroppingRect () {
			...
			const imageRect = this.calculatePosition(this.state.image, dimension.border)
